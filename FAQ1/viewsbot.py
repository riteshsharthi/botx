...
def quession_bookticket(request):
    ...
    if request.method == 'POST':
        ...
        if utter and user_id:
            context_data = obj_counter_logic.main(utter, user_id)
        ...
        for question in context_data["chat_res"]["reply_text"]:
            ...
            if question == "Your data has been proceed. Please wait ...":
                sleep(4)
                con_dict['api_data'] = "your data is 10,00,000 RS"

            if question == "Is there any thing else you want to know. Yes or No?":
                con_dict['menu_task']=['_Yes_','_No_']
...
########################################## Extracted DATA ####################################################################
            ...
            # if context_data['chat_req']['chat_attributes']['task']:
            #     Task_Id =IntentToTask.objects.get(Intent_Name=context_data['chat_req']['chat_attributes']['task'])
            #
            #     TaskToEntityUser.objects.create(id=(max(i.id for i in TaskToEntityUser.objects.all()) + 1 and 1),
            #                                     Task_Id=Task_Id.id, Entity_Name=T_Table.Entity_Name,
